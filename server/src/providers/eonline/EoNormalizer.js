/**
 * Нормализатор E-Online: HTML карточек (`videos__item`) → структурные данные.
 *
 * Карточки `data-json='{…}'`:
 ...
 *
 * Ещё `data-json='JSON'` содержит URL-эскейпы вида `+` — HTML-entities не
 * бывает, значение декодируется через JSON.parse (regex требует неленивую
 * кавычку-сессию).
 */
export class EoNormalizer {
