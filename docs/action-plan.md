...
## Сессия 8.5 (2026-08-08) — Telegram: продление /grant «@ник 08.08.26 P» ✅
- [x] Новый формат `/grant <@ник|id|токен> ДД.ММ.ГГ [P]` (дата конца, конец дня) + `/grant @ник 30` (дни) — бот, тесты, деплой VPS `c1aff1a`
- [x] Поиск пользователя по нику устойчив: транслит кириллицы, любой регистр, «ё»→«е», подчёркивания/дефисы/пробелы,
      обратная совместимость со старыми слитными slugs; `slugFrom` пробел→`-` (`ИванИванов` → `ivan-ivanov`)
- [x] Not-found: подсказка `Точный @ник смотри в /list, или используй id/токен`
- Тесты: 212 (211 pass + 1 skip); live `/health` ok, новый код подтверждён на VPS (`/opt/maniya-online/server/src/telegram/bot.js`)
