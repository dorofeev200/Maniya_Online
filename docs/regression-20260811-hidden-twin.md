...
- **Аккаунт skaz-кластера НЕ при чём**: prod использует ту же учётку, что и работающий E-Online
  (значения удалены из git — SECURITY-001). Прямой probe `lite/*` с ним отдаёт полные данные
  (alloha 13KB call, filmix 5KB play, …).
