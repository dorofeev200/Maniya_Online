...
// Shadow отвечает proxy-URL на loopback (127.0.0.1:3210) — при внешнем запуске
// переписываем на публичный адрес shadow, чтобы цепочка шла через те же ноды.
let REWRITE_LOOPBACK = false;
if (process.env.FPG_REWRITE) {
  const up = new URL(BASE);
  const to = `${up.hostname}:${up.port}`;
  const from = `127.0.0.1:${up.port}`;
  REWRITE_LOOPBACK = (url) => url.replaceAll(`http://${from}`, `http://${to}`);
}
if (!TOKEN) {
  try {
...
async function getJSON(url) {
  const res = await fetch(rewrite(url), { headers: { 'User-Agent': 'fpg-matrix' }, signal: AbortSignal.timeout(15000) });
...
async function rangeFetch(url) {
  try {
    const res = await fetch(rewrite(url), { headers: { Range: 'bytes=0-1023', 'User-Agent': 'fpg-matrix' }, signal: AbortSignal.timeout(15000) });
    const ct = res.headers.get('content-type') || '';
    const bytes = Buffer.from(await res.arrayBuffer());
    return { status: res.status, ct, bytes };
  } catch (e) { return { status: 'ERR', err: String(e).slice(0, 60), bytes: Buffer.alloc(0) }; }
}
function rewrite(url) { return REWRITE_LOOPBACK ? REWRITE_LOOPBACK(url) : url; }
